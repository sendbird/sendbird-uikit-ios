...
class MessageTemplateRenderer: UIView {
    ...
    var theme: SBUMessageTemplateTheme
    ...
    init?(
        ...
        guard let decoded = try? JSONDecoder().decode(MessageTemplateData.self, from: data) else { return nil }
        
        self.version = decoded.version ?? 0
        guard let body = decoded.body else { return nil }
    ...
    func renderBoxItems(_ item: SBUMessageTemplate.Box, parentView: UIView) {
        ...
        if item.layout == .row {
            ...
            self.rendererConstraints += sideView2.sbu_constraint_equalTo_v2(
                ...
                top: (item.viewStyle?.margin?.top ?? 0.0) + prevItemBottomMargin
            )
        }
    }
    ...
    func renderText(item: SBUMessageTemplate.Text,
        ...
        if let textStyle = item.textStyle {
            var fontSize = self.theme.textFont.pointSize
            ...
            label.font = UIFont.systemFont(
                ofSize: self.theme.textFont.pointSize,
                weight: .regular
            )
            label.textColor = self.theme.textColor
    ...
    func renderTextButton(item: SBUMessageTemplate.TextButton,
        ...
        // Text Style
        if let textStyle = item.textStyle {
            var fontSize = self.theme.textButtonFont.pointSize
            if let size = textStyle.size {
                fontSize = CGFloat(size)
            }
            ...
            if let textColor = textStyle.color {
                ...
                textButton.setTitleColor(self.theme.textButtonTitleColor, for: .normal)
            }
        } else {
            textButton.titleLabel?.font = self.theme.textButtonFont
            textButton.setTitleColor(self.theme.textButtonTitleColor, for: .normal)
        }
    ...
    func renderViewStyle(with item: SBUMessageTemplate.View, to view: UIView) {
        guard let viewStyle = item.viewStyle else {
            ...
            if item is SBUMessageTemplate.Box {
                view.backgroundColor = self.theme.viewBackgroundColor
            }
            return
        }
        ...
        if let backgroundColor = viewStyle.backgroundColor {
            ...
            if item is SBUMessageTemplate.TextButton {
                view.backgroundColor = self.theme.textButtonBackgroundColor
            } else {
                view.backgroundColor = self.theme.viewBackgroundColor
            }
        }
        ...
        if let borderRadius = viewStyle.radius {
            view.roundCorners(corners: .allCorners, radius: CGFloat(borderRadius))
        }
    }
